...
# Set matplotlib backend before any imports that might use matplotlib
import matplotlib
matplotlib.use('Agg')

# Ensure current directory is first in Python path
...
def main(Config, RUN):
    ...
    Config.gradient_clip_norm = getattr(Config, "gradient_clip_norm", None)
    Config.enable_gradient_monitor_console = getattr(Config, "enable_gradient_monitor_console", False)
    Config.use_wandb = getattr(Config, "use_wandb", False)
    Config.wandb_project = getattr(Config, "wandb_project", None)
    Config.wandb_run_name = getattr(Config, "wandb_run_name", None)
    Config.wandb_entity = getattr(Config, "wandb_entity", None)
    Config.wandb_tags = getattr(Config, "wandb_tags", None)
    Config.wandb_config = getattr(Config, "wandb_config", None)
    if Config.wandb_config is None:
        meta_data = getattr(Config, "meta_data", {})
        exp_name = meta_data.get("exp_name", None) if isinstance(meta_data, dict) else getattr(Config, "exp_name", None)
        Config.wandb_config = {
    ...
    diffusion_config = utils.Config(
        ...
        use_improved_meanflow=getattr(Config, "use_improved_meanflow", False),
    )
    ...
    trainer_config = utils.Config(
        ...
        enable_gradient_monitor_console=getattr(Config, "enable_gradient_monitor_console", False),
