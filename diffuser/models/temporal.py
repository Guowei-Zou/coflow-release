...
class TemporalSelfAttention(nn.Module):
    ...
    def forward(self, x, time, attn_scale=None, return_attn_weights=False):
        ...
        if self.residual:
            scale = attn_scale if attn_scale is not None else 1.0
            out = x + scale * self.gamma * out
        if return_attn_weights:
            return out, attn  # attn shape: (heads, batch, agents, agents)
        return out
