...
def apply_conditioning(x, conditions):
    ...
    # Use torch.where instead of in-place indexing to preserve autograd graph
    # for DPPO-style partial fine-tuning (gradient through last denoising steps)
    cond_values = conditions["x"]
    x = torch.where(cond_masks, cond_values, x)
    ...
    if "player_idxs" in conditions.keys():
        if x.shape[-1] < 4:  # pure position information w.o. player info
            ...
            new_x = x.clone()
            new_x[:, :, :, 0] = conditions["player_idxs"]
            new_x[:, :, :, -1] = conditions["player_hoop_sides"]
            x = new_x
